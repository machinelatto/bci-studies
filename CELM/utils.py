...
def prepare_data_subjects(
    ...
    X_train, X_test_full, y_train, y_test_full = split_train_test_subjects(
        X, y, test_subject
    )

    if validation_split:
        X_test, X_val, y_test, y_val = train_test_split(
            X_test_full, y_test_full, test_size=0.2, random_state=42
        )
        train_data = TensorDataset(
            torch.tensor(X_train, dtype=torch.float32).to(device),
            torch.tensor(y_train, dtype=torch.long).to(device),
        ...
        test_data = TensorDataset(
            torch.tensor(X_test_full, dtype=torch.float32).to(device),
            torch.tensor(y_test_full, dtype=torch.long).to(device),
        )
        train_loader = DataLoader(train_data, batch_size=train_batch_size, shuffle=True)
        test_loader = DataLoader(test_data, batch_size=test_batch_size, shuffle=True)
        print(f"Training dataset size: {len(y_train)}")
        print(f"Test dataset size: {len(y_test_full)}")
        return train_loader, test_loader
